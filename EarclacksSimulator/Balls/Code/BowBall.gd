...
@onready var ArrowScene = preload("res://Balls/Projectiles/BowArrow.tscn")
...
var Name="BowBall"
var WeaponDetectName="BowWeaponDetector"
var BallNum=0
...
func Arrow():
	$ShootSound.play()
	for body in $BowWeaponDetector.get_overlapping_bodies():
		if body.is_in_group("Walls"):
			return
	var Arrow = ArrowScene.instantiate()
	get_tree().current_scene.add_child(Arrow)
	Arrow.ReadBowName(self)
	Arrow.global_position = $BowWeaponDetector/ArrowSpawnLocation.global_position
	Arrow.look_at($BowWeaponDetector/ArrowFacing.global_position)
func _ready():
	$Ball.play("Normal")
	...
	Global.MapName.NumOfLabelsVisible(2,"Bow")
	Global.MapName.ChangeOneLabel("Arrows: "+str(Arrows),"Bow")
	Global.MapName.ChangeTwoLabel("Damage Dealt: "+str(DamageDealt),"Bow")
	$ArrowCooldown.start()
...
func _process(_delta: float) -> void:
	ShootInterval=clamp(ShootInterval,0.01,1000000)
	...
	Global.MapName.ChangeOneLabel("Arrows: "+str(Arrows),"Bow")
	Global.MapName.ChangeTwoLabel("Damage Dealt: "+str(round(DamageDealt*10)/10),"Bow")
	if HP<=0:
		Global.Loser=Name
func _physics_process(delta):
	$BowWeaponDetector.rotate(deg_to_rad(delta*150*RotationDirection))
...
func _on_bow_weapon_detector_body_entered(body: Node2D) -> void:
...
func _on_bow_weapon_detector_area_entered(area: Area2D) -> void:
...
func _on_arrow_cooldown_timeout() -> void:
	if ArrowsLeft>0:
		ShootInterval=clamp(ShootInterval,0.01,1000000)
		ShootInterval=1.5/Arrows
		ArrowsLeft-=1
		$ArrowCooldown.wait_time=ShootInterval
		$ArrowCooldown.start()
		if HP>0:
			Arrow()
	elif $NextArrowCooldown.is_stopped()==true:
		$NextArrowCooldown.start()


func _on_next_arrow_cooldown_timeout() -> void:
	ArrowsLeft=Arrows
	$NextArrowCooldown.stop()
