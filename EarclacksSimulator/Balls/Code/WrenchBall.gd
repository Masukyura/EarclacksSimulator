...
var Turrets=0
var TurretLifetime=30
var DamageDealt=0
var HP=Global.HP

var Name="WrenchBall"
var WeaponDetectName="WrenchWeaponDetector"
var BallNum=0
@onready var TurretScene = preload("res://Balls/Projectiles/WrenchTurret.tscn")
...
func _ready():
	...
	Global.MapName.NumOfLabelsVisible(3,"Wrench")
	Global.MapName.ChangeOneLabel("Turrets: "+str(Turrets),"Wrench")
	Global.MapName.ChangeTwoLabel("Damage Dealt: "+str(DamageDealt),"Wrench")
	Global.MapName.ChangeThreeLabel("Turret Lifetime: "+str(TurretLifetime)+"sec","Wrench")
...
func _physics_process(delta):
	$WrenchWeaponDetector.rotate(deg_to_rad(delta*300*RotationDirection))
...
func _on_wrench_weapon_detector_body_entered(body: Node2D) -> void:
	if body!=self && body.is_in_group("Balls") && HP>0:
		body.TakeDamage(1)
		body.DamageTick(0.2)
		var Turret = TurretScene.instantiate()
		Turret.Lifetime=TurretLifetime
		get_tree().current_scene.add_child(Turret)
		Turret.GetWrenchName(self)
		Turret.global_position = $WrenchWeaponDetector/TurretLocation.global_position
		RotationDirection*=-1
		DamageDealt+=1
...
func _on_wrench_weapon_detector_area_entered(area: Area2D) -> void:
