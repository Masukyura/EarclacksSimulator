...
var SpinSpeed=10
var DamageTickTime=0.1
var DamageDealt=0
var HP=Global.HP

var Name="DaggerBall"
var WeaponDetectName="DaggerWeaponDetector"
var BallNum=0
...
func _ready():
	...
	Global.MapName.NumOfLabelsVisible(2,"Dagger")
	Global.MapName.ChangeOneLabel("Spin Speed: "+str(SpinSpeed),"Dagger")
	Global.MapName.ChangeTwoLabel("Damage Dealt: "+str(DamageDealt),"Dagger")
...
func _physics_process(delta):
	$DaggerWeaponDetector.rotate(deg_to_rad((delta*300*RotationDirection+(delta*50*(clamp(SpinSpeed,10,750))*RotationDirection))))
...
func _on_dagger_weapon_detector_body_entered(body: Node2D) -> void:
	if body!=self && body.is_in_group("Balls") && HP>0:
		body.TakeDamage(1)
		body.DamageTick(DamageTickTime)
		DamageTickTime-=0.0005
		DamageTickTime=clamp(DamageTickTime,0.05,0.1)
		RotationDirection*=-1
		DamageDealt+=1
		$HitSound.play()
		SpinSpeed+=5
...
func _on_dagger_weapon_detector_area_entered(area: Area2D) -> void:
